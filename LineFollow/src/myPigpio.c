/**************************************************************
* Class:: CSC-615-01 Spring 2025
* Name:: Ethan Lam, Kevin Chuong, Hiro Eernisse, Andy Garcia
* Student ID:: 921993953, 922176863, 923217227, 921107769
* Github-Name:: RNDME1
* Project:: Term Project - Drive On
*
* File:: myPigpio.c
*
* Description:: pigpio library functions for quick and easy
*   access for things like initalizing pins
*
**************************************************************/
#include "myPigpio.h"


// Initializes the PiGPIO DMA setup, call before use
int initializePiGPIO(){

    // Initalise gpio library to let every other gpio function run
    int gpioResult = gpioInitialise();
    if(gpioResult < 0){
        // GPIO FAILED
        printf(
            "ERROR: GPIO failed to initalize\nError code: %d\n", 
            gpioResult
        );
        return 1;

    } 
    return 0;
}

// Terminates the DMA, call before exiting
int terminatePiGPIO(){
    gpioTerminate();
    return 0;
}

/*
* Initalizes a given pin to input/output 
* pin: gpio pin
* pinMode: input/output
* returns: 0 on success, 1 on bad pin mode, -1 on gpio failure
*/
int initPin(unsigned int pin, unsigned pinMode){
    int gpioResult = -1;
    if(pinMode != PI_INPUT && pinMode != PI_OUTPUT){
        printf("Error: Invalid pin mode(0-1) given: %d\n", pinMode);
        return 1;
    }

    gpioResult = gpioSetMode(pin, pinMode);

    if(gpioResult != 0){
        switch (gpioResult){
            case PI_BAD_GPIO:
                printf(
                    "%s%d%s%d\n", 
                    "ERROR, set mode failed due to bad GPIO pin on pin ", 
                    pin, 
                    "\nError code: ", 
                    gpioResult
                );
                break;
            
            
            case PI_BAD_MODE:
                printf(
                    "%s%d%s%d\n", 
                    "ERROR, bad mode for GPIO pin ", 
                    pin, 
                    "\nError code: ", 
                    gpioResult
                );
                break;
            
    
            default:
                printf(
                    "%s%d%s%d\n", 
                    "ERROR, unexpected error occured on pin ", 
                    pin, 
                    "\nError code: ", 
                    gpioResult
                );            
        }
        
        return -1;
    
    }
    
    return 0;
}

/*
*   Initalizes a given pin to a button
*   pin: pin number
*   returns: 0 on success, 1 on failure
*/
int initBtn(unsigned int pin){
    if(initPin(pin, PI_INPUT) != 0){
        return 1;
    }
    // Slight delay to ensure it can properly read
    usleep(100);
    if(setPinPull(pin, PI_PUD_DOWN) != 0){
        return 1;
    }

    return 0;
}

int cleanBtn(unsigned int pin){
    initPin(pin, PI_OUTPUT);

    return 0;
}

/*
*   Awaits until a given pin has read a high output
*   pin: GPIO pin
*   returns 0 on high read
*/
int awaitButtonPress(unsigned int pin){
    while(gpioRead(pin) == 0){
        usleep(10);
    }

    return 0;   
}

/*
*   Awaits until a given pin has read the opposite of what is give
*   pin: GPIO pin
*   start: starting state (0 or 1)
*   returns 0 on high read, 1 on invalid starting
*/
int awaitStateChange(unsigned int pin, unsigned int start){
    if(start > 1 || start < 0) {
        return 1;
    }
    while(gpioRead(pin) == start){
        usleep(1000);
    }

    return 0;
}

/*
*   Reads the given pin
*   pin: GPIO pin
*   returns 0 or 1
*/
int readPin(unsigned int pin){
    return gpioRead(pin);   
}

/*
* Sets a given pin to a given pin pull mode
* pin: GPIO pin
* mode: Pull mode
* returns 0 on success, 1 on invalid pull mode
*/
int setPinPull(unsigned int pin, unsigned mode){
    if(mode < 0 || mode > 2){
        printf("Error: mode given is out of bounds(0-2): %d\n", mode);
        return 1;
    }
    gpioSetPullUpDown(pin, mode);
    return 0;
}

/*
* Initalizes a given pin to input/output 
*
* @param pin: given gpio pin
* @param pinMode: Selected pin mode
*
* returns: 0 on success, 1 on bad pin mode, -1 on gpio failure
*/
int setPWM(unsigned int pin, unsigned int power){
    int feedback = 0;

    if(power > 255){
        power = 255;
    }

    feedback = gpioPWM(pin, power);

    switch(feedback){
        case PI_BAD_USER_GPIO:
            printf("ERROR, setPWM failed! Bad PI user GPIO detected!\n");
            break;
        
        case PI_BAD_DUTYCYCLE:
            printf("ERROR, setPWM failed! Bad Dutycycle!\n");
            break;
        
        default: 
            usleep(50000); // Ensures pwm change goes through
            
    }

    return feedback;
}

uint32_t createDelay(unsigned int time){
    return gpioDelay(time);
}

uint32_t createDelaySec(unsigned int time){
    return gpioDelay(time * 1000000);
}
