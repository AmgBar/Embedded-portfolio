...
int initRGBSensor(){
    ...
    if(initPin(pin, PI_OUTPUT) || setPWM(pin, 225)){
        return 1;
    }

    if(initTCS34725(TCS34725_CMD_BIT)){
        return 1;
    }
...
*
* returns: RGB color enum on success, null on failure
*/
enum ColorEnum getColorEnum(){
    enum ColorEnum r = getEnum();
    return r; 
}
